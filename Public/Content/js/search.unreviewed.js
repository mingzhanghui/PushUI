...
$(document).ready(function() {
    ...
    $("#dlg_specialepisode").dialog({
        ...
        open: function(event, ui) {
            ...
            $("#js-thumb-" + mediatypeid).off("click").on("click", function() {
                $("#eps_thumb_" + mediatypeid).trigger("click");
            });
            $("#js-poster-" + mediatypeid).off("click").on("click", function() {
                $("#eps_poster_" + mediatypeid).trigger("click");
            });
            $("#js-thumboid-" + mediatypeid).val(oid);
            $("#js-posteroid-" + mediatypeid).val(oid);
...
function renderMediaAppendix(data, mediatypeid) {
    ...
    var html = "";
    if (data.length == 0) {
        html = "<tr class='dumb'><td colspan='6'>没有上传图片附件</td></tr>";
    } else {
    ...
    $.ajax({
        ...
        if (1 == resp.file) {
            ...
            if (0 == $tbody.children("tr").length) {
                $tbody.html('<tr><td colspan="6">没有上传图片附件</td></tr>');
            }
...
function dlg_specialepisode_open_handler() {
    ...
    $.ajax({
        ...
        var renderPics = function(data) {
            Array.prototype.forEach.call(data.pic, function (elem, i) {
                // var url = g_prefix + elem.url;
                var url = elem.url;
                if (elem.appendixtypeid == '1') {
                    $("#js-thumb-" + mediatypeid).css({"background-image": "url('" + url + "')"});
                } else if (elem.appendixtypeid == '2') {
                    $("#js-poster-" + mediatypeid).css({"background-image": "url('" + url + "')"});
                } else {
                    console.error("unexpected appendixtypeid");
                }
            });
        };
        if (typeof g_prefix === "undefined" || g_prefix === null || g_prefix === '') {
            $.ajax({
                type: "GET",
                url:getEditController() + '/getAppendixURLPrefix'
            }).done(function(json) {
                g_prefix = json.path;  // /PushUI/resource/appendix/
                renderPics(data);
            });
        } else {
            renderPics(data);

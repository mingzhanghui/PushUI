    ...
    $("#dialog-1").dialog({
        ...
        open: function(event, ui) {
            ...
            $.ajax({
                type: 'get',
                data: {'id':id},
                url: baseurl + 'getPausePut'
            }).done(function(obj) {
                Object.keys(obj).forEach(function(key, idx) {
                    $('#ad_' + key).val(obj[key]);
                });
            }).fail(function(jqXHR, textStatus, errorThrown) {
                alert('getPausePut ajax error: ' + errorThrown);
            });
        ...
        buttons: [
            {
                ...
                click: function(event, ui) {
                    ...
                    input.value = 'AdPauseadpushstatus';
                    form.appendChild(input);
 ...
 */
function cancelPush() {
    var checkboxes = document.getElementsByName("PutPauseID[]");

    var v = [];
    Array.prototype.forEach.call(checkboxes, function(checkbox, i) {
        if (checkbox.checked) {
            // @table:  MBIS_Server_Ad_PauseAdPushStatus
            // @column: ID

...
class LoginController extends Controller {
    public function index($refer='') {
        ...
        if (isset($_SESSION['username']) && $_SESSION['subscribe']) {
            $refer = I('get.refer');
            $refer = ($refer === '') ? U('Subscribe/Index/index') : urldecode($refer);
            redirect($refer);
        }
        ...
        if(IS_POST) { //登录验证
            ...
            if(0 < $uid) { // 登录成功
                // 判断进入权限管理系统的权限
                if ( !$User->permission($uid, 'subscribe') ) {
                    $this->error($username.'没有权限访问业务管理');
                }
    ...
    public function logout() {
        ...
        if ($config->hasLogin()) {
            ...
            redirect(U('Subscribe/Login/index'));
        } else {
            redirect(U('Subscribe/Index/index'));
        }
    }
